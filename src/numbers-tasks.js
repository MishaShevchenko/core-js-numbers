...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // Use the arc cosine function to get the angle
...
function roundToPowerOfTen(num, pow) {
  let powNumb = num / 10 ** pow;
  powNumb = Math.round(powNumb);
  powNumb *= 10 ** pow;
  return powNumb;
}
...
function isPowerOfTwo(num) {
  let n = num; // Create a local variable to perform division
  ...
  while (n > 1) {
    ...
    n /= 2; // Divide the local variable by 2 until it becomes 1 or not divisible by 2
  }

  return true; // If the loop completes, it's a power of two
}
...
function getNumberValue(number) {
  // Check if the input is an instance of the Number object
  if (number instanceof Number) {
    // Extract the primitive value using valueOf()
    return number.valueOf();
  }

  // If it's not a Number object, return the input as is
  return number;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 + x2 + x3;
  return roundToFixed(sum, 1); // Round to 1 decimal place
}
...
function getRandomInteger(min, max) {
  ...
  const [localMin, localMax] = min > max ? [max, min] : [min, max];
